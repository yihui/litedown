...
target output format. For those who are familiar with R Markdown, you can think
of the function `litedown::fuse()` as the new `knitr::knit()`,
`litedown::fiss()` as the new `knitr::purl()`, and `litedown::mark()` as the new
Pandoc. @fig:fuse-flow shows the conversion process from R Markdown to a target
format.

We will focus on `fuse()` in this chapter, and leave `mark()` to @chp:syntax and
@chp:mark.

::: callout-note
Unless you use **litedown** programmatically (i.e., via R code), you should
rarely need to call `fuse()` or `mark()` directly. Instead, please consider
using `litedown::roam()` to preview or render your files (@sec:live-preview),
which will call `fuse()` and `mark()` behind the scenes.
:::
...
When we `fuse()` the document, the program code will be executed, giving us the
results below:
...
short, and write them in pipe comments when they are too long.
...
### All chunk options {#sec:chunk-options}
...
The Markdown output from a code chunk follows a structure like this (illustrated
in @fig:attr-kanban):
...
```{mermaid}
#| attr-kanban, echo = FALSE, fig.env = '.figure .box',
#| fig.cap = 'An illustration of the output structure of a code chunk.'
...
-   `attr.plot`: Attributes for plots. If a code chunk produces multiple plots,
    ...
    you may use `attr.plot = sprintf('width="%s"', c('40%', '60%'))` to set the
    width of the first plot to `40%`, and the second plot to `60%`.
...
changed[^01-fuse-1] and the external dependencies used by the chunk have not
changed, either, otherwise the cache will be invalidated and the results will be
re-computed.

[^01-fuse-1]: Cosmetic changes such as adding/deleting spaces and blank lines or
...
current run, and (lazy-)loaded[^01-fuse-2] in the next run, so they can be
available to later code chunks in the document.

[^01-fuse-2]: Lazy-loading means that the value of a variable will not be read
...
#### `ref.label`: chunk references {#sec:option-ref-label}

A code chunk can reuse code from other code chunks via the option `ref.label`.
It takes a vector of chunk labels, and will copy code from these chunks.

Using chunk references makes it possible to avoid copying code from one chunk to
another manually. You can also compose code freely from other chunks.
...
#### `strip.white`: leading / trailing blank lines in code {#sec:option-strip-white}
