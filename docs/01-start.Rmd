...
<p><button class="new-fuse">Fuse</button> <button class="new-fuse new-preview">Run</button></p>
```
:::
...
You can learn more about the `Fuse` and `Run` buttons in @sec:live-preview.
Basically, the `Fuse` button renders a document to an output file, whereas the
...
seriously brainwashed by nursery rhymes that Humpty Dumpty came to my mind when
I was thinking about an example to explain computational documents. Don't be
surprised if you run across Blippi and garbage trucks somewhere in this book.
:::
::::
...
he will never send a pull request to **litedown**.
