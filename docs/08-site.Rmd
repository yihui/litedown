...
If you use RStudio, you can create a new litedown book or website project from
the menu `File` > `New Project` > `New Directory` > `Project using litedown`.
You will be able to select the project type (book or site) from the dropdown
menu.

Alternatively, you can create a minimal project with the internal helper
`litedown:::proj_skeleton()`:

``` r
litedown:::proj_skeleton("my-book", type = "book")
litedown:::proj_skeleton("my-site", type = "site")
```

The book skeleton contains an `index.Rmd` (the first chapter with YAML
metadata), a sample chapter `01-intro.Rmd`, and a `_litedown.yml` config file.
The site skeleton contains `index.Rmd`, `about.Rmd`, and `_litedown.yml`. You
can build the book with `fuse_book()` and the site with `fuse_site()`. The
configuration options are documented in later sections of this chapter.
...
-   The default CSS assets are
    `{r} xfun::join_words(litedown:::site_css, before = "\x60")`, and the default
    JS assets are `{r} xfun::join_words(litedown:::site_js, before = "\x60")`. In
    other words, each page uses the article format (@sec:article) by default.
...
action](https://github.com/yihui/actions/blob/main/litedown-site/action.yml) in the
**actions** repository, and you can add a step `uses: yihui/actions/litedown-site@HEAD`
...
The action `yihui/actions/litedown-site` supports a few options that you can configure
...
if (!dir.exists('../../actions'))
  system('git clone --depth 1 https://github.com/yihui/actions.git ../../actions')
inputs = xfun::yaml_load(readLines('../../actions/litedown-site/action.yml'), use_yaml = FALSE)$inputs
cat(sprintf(
...
template](https://github.com/yihui/actions/tree/main/litedown-site) from the **actions** repo,
which contains the following `.Rmd` files (explained in @tab:pkg-site):

```{r}
list.files('../../actions/litedown-site/', '[.]Rmd$')
```
