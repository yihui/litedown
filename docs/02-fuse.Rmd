...
::: callout-caution
Setting an option *both* globally (via `reactor()`) and locally (in the chunk
header) *in the same chunk* will not work as you might expect: the global value
will be lost after this chunk. This is because local chunk options are applied
on top of global options before a chunk is evaluated, and restored to their
previous global values after the chunk is done, which will override any global
value you set inside the chunk. For example, do *not* do this:

```` md
```{r, setup}
#| dev = 'jpeg'
litedown::reactor(dev = 'jpeg')  # will NOT persist to later chunks
```
````

If you want to set an option globally, set it only via `reactor()` and do not
also provide it as a local chunk option.
:::
...
::: callout-tip
If the default device is not available on your system (e.g., R was built without
Cairo support, so `png()` cannot open a device), you can set an alternative
device globally in a setup chunk, e.g., `litedown::reactor(dev = ragg::agg_png)`.
Remember to set `dev` *only* via `reactor()` and not also as a local chunk
option, or the global value will be lost (see the caution in @sec:reactor). If
this setup chunk does not itself generate any plots, also set `fig.path = NA`
(@sec:chunk-options) for it so that `fuse()` will not try to open the
(unavailable) default device for this chunk.
:::
...
-   `2`: Print all invisible values in the code, except for invisible `NULL`
...
The `exec` engine runs an arbitrary command and captures its output. It writes
the chunk body to a temporary file and executes:

``` sh
command [args1] file [args2]
```

where `command` is specified by the `command` chunk option, `file` is the
temporary file containing the chunk body, and `args1`/`args2` are optional chunk
options for extra arguments before/after the file path. For example:
...
The shell engines `sh`, `bash`, `zsh`, and `powershell` are shortcuts based on
`exec`, using the engine name as the command directly, e.g.,
...
These engines also work inline, e.g., `` `{bash} date` `` will insert the
output into the text.

For commands that need flags before the file path, use `args1`. For example,
`awk` requires `-f` to read its program from a file (i.e., `awk -f file`):

```` md
```{exec, command = 'awk', args1 = '-f'}
...
-   The document parser of **knitr** is based solely on regular expressions and
    ...
    contexts. The parser of **litedown** is based on cmark-gfm, which is
