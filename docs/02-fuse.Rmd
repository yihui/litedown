...
You can set an option both globally (via `reactor()` in the chunk code) and
locally (in the chunk header) in the same chunk. The global value will persist
to later chunks, while the local value applies only to the current chunk, e.g.:

```` md
```{r, setup}
#| dev = 'png'
litedown::reactor(dev = 'svg')  # 'svg' persists to later chunks
```
````
