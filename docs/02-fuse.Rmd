...
### The exec engine {#sec:engine-exec}

The `exec` engine can be used to run arbitrary commands via `system2()` and
capture the output. The first line of the chunk body is the command to run, and
the remaining lines are passed to the command as standard input, e.g.,
...
The shell engines `sh`, `bash`, and `zsh` are shortcuts based on `exec`, using
the engine name as the command directly, e.g.,
...
You can override the command via the chunk option `command`, e.g.,
