...
```{r, setup}
#| dev = 'jpeg'
litedown::reactor(dev = 'svg')  # will NOT persist to later chunks
```
````
...
device globally in a setup chunk, e.g., `litedown::reactor(dev = 'svg')`. If
this setup chunk does not itself generate any plots, also set `fig.path = NA`
(@sec:option-fig) for it, so that `fuse()` will not try to open the
