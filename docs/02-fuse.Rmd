...
The `exec` engine can be used to run arbitrary commands via `system2()` and
capture the output. The chunk option `command` is required to specify the
command to run, and the chunk body is the code to be executed, e.g.,
...
You can pass additional arguments to the command via the `args1` chunk option
(inserted before the script file path) or `args2` (after it). For example,
`awk` requires the `-f` flag to read its program from a file:
