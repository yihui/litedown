...
Pandoc's Markdown), e.g., 2^10^ and H~2~O. The `text` is parsed as inline
Markdown, so it may contain other formatting (e.g., `a^b*c*^` gives a
superscript containing emphasized text). The delimiters follow the same
"flanking" rules as the `*` and `_` used for emphasis: an opening delimiter must
be immediately followed by non-space content and a closing delimiter must be
immediately preceded by non-space content, so `a^b c^` is not a superscript
(there is a space next to the closing `^`), and a delimiter that sits right next
to punctuation may be treated literally. Note that GFM supports striking out
text via `~text~`, but this feature has been disabled and replaced by the
feature of subscripts in **litedown**. To strike out text, you must use a pair
of *double* tildes.
...
Alternatively, a bibliography database can be an R script (with the file
extension `.R`) that contains `bibentry()` calls, e.g., `inst/REFERENCES.R` in
an R package. You can mix `.bib` and `.R` files, e.g.,

``` yaml
bibliography: ["papers.bib", "inst/REFERENCES.R"]
```
...
Note that the `.R` bibliography format is only supported for HTML output. For
LaTeX/PDF output, the bibliography is processed by external tools (e.g., BibTeX
or Biber) via `\bibliography{}` or `\addbibresource{}`, which only accept `.bib`
files.
