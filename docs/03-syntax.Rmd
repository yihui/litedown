...
In addition to the basic features above (supported by cmark-gfm), the
...
(there is a space right before the closing `^`), and a delimiter that sits right
next to punctuation may be treated literally. A single `~` is used for
subscripts in **litedown**; to strike out text, use a pair of *double* tildes,
i.e., `~~text~~`.
