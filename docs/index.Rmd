...
xfun::pkg_bib(c('litedown', 'xfun', 'rbibutils'), 'packages.bib')
```
...
`litedown::mark()`, which uses the [cmark-gfm](https://github.com/github/cmark-gfm)
C library (bundled with the package) instead of Pandoc.

The cmark-gfm library follows the GFM (GitHub Flavored Markdown) spec,
...
[^2]: In fact, `xml,` `man`, `text`, and `commonmark` output formats are
    supported (thanks to the cmark-gfm library), but perhaps they are not
...
lightweight, with only one R package dependency: **xfun** [@R-xfun]. The
Markdown parser and renderer are provided by the
[cmark-gfm](https://github.com/github/cmark-gfm) C library bundled with the
package.
...
Just to give you a better idea about the "lightweight", as of today:

-   As the Markdown converter, Pandoc 3.8.3 binary is about 186Mb, and the 
    **litedown** binary is under 1Mb on my macOS.
...
footprint (the total file size of **litedown** and **xfun**),
you can:
...
Due to the fact that the parser is based on cmark-gfm (instead of solely on
