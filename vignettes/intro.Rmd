...
The **litedown** package is a lightweight implementation of R Markdown built on
top of **commonmark**. R Markdown is rendered via `litedown::fuse()`, which is
similar to `rmarkdown::render()` and `knitr::knit()` but does not require
**rmarkdown** or **knitr**. Markdown is rendered via `litedown::mark()`, which
uses **commonmark** instead of Pandoc. The primary output formats are HTML and
LaTeX.
...
| The document parser is based on regular expressions and not robust.                                                                       | The parser is based on **commonmark**, which is more robust and makes it easier to write verbatim elements. |
...
re-implementing **knitr** or Pandoc. Some features may never be re-implemented,
